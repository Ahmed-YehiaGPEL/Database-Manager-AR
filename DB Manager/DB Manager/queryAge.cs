...
{
    public partial class queryAge : Form
    {
        public queryAge()
        {
            InitializeComponent();
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            if (isMulti == false)
            {
                queryString = "SELECT * from pltg WHERE \"العمر\"" + cmbAge1.SelectedItem.ToString() + " " + numAge1.Value.ToString();
                System.Threading.Thread loadThread = new System.Threading.Thread(loadTable);
                loadThread.Start();
            }
            else
            {
                queryString = "SELECT * from pltg WHERE \"العمر\"" + cmbAge1.SelectedItem.ToString() + " " + numAge1.Value.ToString() +" AND \"العمر\" "+cmbAge2.SelectedItem + " " +numAge2.Value.ToString();
                System.Threading.Thread loadThread = new System.Threading.Thread(loadTable);
                loadThread.Start();
            }
        }
